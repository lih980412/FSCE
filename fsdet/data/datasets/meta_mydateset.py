...
__all__ = ["register_meta_mydateset"]
...
def load_coco_json(json_file, image_root, metadata, dataset_name):
    ...
    if is_shots:
        fileids = {}
        split_dir = os.path.join('datasets', 'my_dataset_split')
        ...
        for idx, cls in enumerate(metadata['thing_classes']):
            ...
            json_file = "D:\\UserD\\Li\\FSCE-1\\" + json_file
...
def register_meta_mydateset(name, metadata, imgdir, annofile):
    ...
    MetadataCatalog.get(name).set(
        json_file=annofile, image_root=imgdir, evaluator_type="mydateset",
