...
from .transform import ExtentTransform, ResizeTransform, SaltPepper, Gaussian
...
class RandomSaltpepper(TransformGen):
    """
    Flip the image horizontally with the given probability.
    ...
    def __init__(self, prob=0.5, percetage=0.000000001):
        """
        Args:
            prob (float): probability of saltpepper.
            percetage (float): percetage of saltpepper.
        """
        super().__init__()
        self._init(locals())
    ...
    def get_transform(self, img):
        ...
        do = self._rand_range() < self.prob
        if do:
            return SaltPepper(h, w, self.percetage)
        else:
            return NoOpTransform()
...
class RandomBrightness(TransformGen):
    ...
    def get_transform(self, img):
        w = np.random.uniform(self.intensity_min, self.intensity_max)
        return BlendTransform(src_image=img, src_weight=1 - w, dst_weight=w)
...
class img_tohalf(TransformGen):
    ...
    def get_transform(self, img):
        assert img.shape[-1] == 3, "Saturation only works on RGB images"
        return BlendTransform(src_image=grayscale, src_weight=1 - w, dst_weight=w)
        return img_tohalfTransform(src_image=grayscale, src_weight=1 - w, dst_weight=w)
