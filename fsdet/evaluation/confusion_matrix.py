...
class ConfusionMatrix:
    ...
    def __init__(self, number_class, conf=0.25, iou_thres=0.45):
        self.matrix = np.zeros((number_class + 1, number_class + 1))
    ...
    def process_batch(self, detections, labels):  #
        ...
        if x[0].shape[0]:
            ...
            if x[0].shape[0] > 1:
                ...
                matches = matches[np.unique(matches[:, 1], return_index=True)[1]]  # 对gt_bbox_id按iou去重，np.unique除去重複
                matches = matches[matches[:, 2].argsort()[::-1]]                # iou排序
                matches = matches[np.unique(matches[:, 0], return_index=True)[1]]   # 对dt_bbox_id按iou去重
        else:
