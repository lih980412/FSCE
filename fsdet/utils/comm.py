...
def Mosaic(data):
    ...
    for i in range(len(index)):
        temp_img = {}
        temp_img["file_name"] = index[i]["file_name"]
        temp_img["height"] = index[i]["height"]
        temp_img["width"] = index[i]["width"]
        temp_img["image_id"] = index[i]["image_id"]
        p_images.append(temp_img)

        for m in range(len(index[i]["instances"].get("gt_boxes").tensor)):
            temp = {}
            if len(index[i]["instances"].get("gt_boxes").tensor) == 1:
                temp["bbox"] = (np.squeeze(np.asarray(index[i]["instances"].get("gt_boxes").tensor, dtype=int), 0)).tolist()
                temp["category_id"] = index[i]["instances"].get('gt_classes').numpy()[0]
            else:
                temp["bbox"] = (np.asarray(index[i]["instances"].get("gt_boxes").tensor[m], dtype=int)).tolist()
                temp["category_id"] = index[i]["instances"].get('gt_classes').numpy()[m]

            temp["image_id"] = index[i]["image_id"]
            p_ann.append(temp)
    ...
    ann = [mosaic_np[i][0:4] for i in range(len(mosaic_np))]
    # ann = torch.as_tensor(ann, dtype=torch.float32)
    data_temp["instances"].get("gt_boxes").set_tensor(ann)

    cls = torch.as_tensor([mosaic_np[i][4] for i in range(len(mosaic_np))], dtype=torch.int64)
