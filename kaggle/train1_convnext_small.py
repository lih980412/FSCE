...
def map_per_image(label, predictions):
    ...
    try:
        ...
        indice = torch.where(torch.topk(torch.tensor(predictions), k=5, dim=0, largest=True, sorted=True)[1] == label)[
            0]
        if len(indice) == 0:
            return 0.0
...
class WandDIDNet(LightningModule):
    def __init__(self, continuous_scheduler=True, s=30, m=0.3, margin=None, dataloader=None, out_dim=None,
                 centerperclass=None):
        super().__init__()
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, device, scheduler, train_len):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        if enable_amp:
            ...
            if (((step + 1) % CFG.ACCUMULATE_GRAD_BATCHES) == 0) or (step + 1 == train_len ):
                # optimizer the net
                # optimizer.step()
...
class ConvNeXt_S(nn.Module):
    def __init__(self, out_dim, centerperclass, drop_rate=0):
        super().__init__()
        self.model = timm.create_model(CFG.model_name, pretrained=True, drop_rate=drop_rate)
        self.embedding = nn.Linear(self.model.get_classifier().in_features, CFG.EMBEDDING_SIZE)
        self.model.reset_classifier(num_classes=0, global_pool="avg")
        self.metric_classify = ArcMarginProduct_subcenter(CFG.EMBEDDING_SIZE, out_dim, centerperclass)

    def forward(self, images, label=None) -> torch.Tensor:
        features = self.model(images)
        embeddings = self.embedding(features)
        logits = self.metric_classify(embeddings)
...
def main():
    ...
    '3'
    model = ConvNeXt_S(out_dim=N_CLASSES, centerperclass=CFG.CENTERS_PER_CLASS)
    model.to(device)
