...
def main_defect():
    dataDir = r'D:\UserD\Li\FSCE-1\datasets\MoLi\image'
    target_path = r'D:\UserD\Li\FSCE-1\tools\Static\Croped\MoLi_train'
    annFile = r'D:\UserD\Li\FSCE-1\datasets\MoLi\annotations\instances_train.json'
    # root_path = sys.argv[1]
    # support_path = os.path.join(root_path, 'support')
    # if not isdir(support_path):
    #     mkdir(support_path)
    ...
    # for dataType in ['image']: #, 'train2017']:
    # set_crop_base_path = join(root_path, dataType)
    ...
    for img_id, id in enumerate(coco.imgs):
        ...
        for item_id, ann in enumerate(anns):
            ...
            if support_img is None:
                continue
            cv2.imwrite(os.path.join(target_path, str(f'{temp}_' + img['file_name'])), support_img)
            temp += 1
