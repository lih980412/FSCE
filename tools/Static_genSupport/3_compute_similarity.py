...
def plt_(similarity):
    ...
    # 添加描述信息
    plt.xlabel('相似度得分', fontsize=20)
    plt.ylabel('得分统计', fontsize=20)
...
if __name__ == "__main__":
    coco_path = r"D:\UserD\Li\FSCE-1\tools\Static_genSupport\Croped\COCO2014_train"
    dibei_path = r"D:\UserD\Li\FSCE-1\tools\Static_genSupport\Croped\DiBei_analysis\DiBeiNormal"
    PATH = dibei_path

    defect_path = r"D:\UserD\Li\FSCE-1\tools\Static_genSupport\Croped\DiPian_train"
    moli_path = r"D:\UserD\Li\FSCE-1\tools\Static_genSupport\Croped\MoLi_train"
    # csv_path = r"D:\UserD\Li\FSCE-1\tools\Static\Croped\result.csv"
    # ANOTHER_PATH = moli_path

    name_coco = coco_path.split("\\")[-1]
    name_dibei = dibei_path.split("\\")[-1]
    NAME = name_dibei

    name_defect = defect_path.split("\\")[-1]
    name_moli = moli_path.split("\\")[-1]
    ANOTHER_NAME = name_moli
    ...
    coco_list = torch.zeros([1, 9])
    coco_files = os.listdir(PATH)
    for file in coco_files:
        ...
        # concate成list
        coco_list = torch.cat([coco_list, torch.tensor([[ave_coco, std_coco, ju3_coco, u_coco, e_coco,
                                                            asm_coco, con_coco, ent_coco, idm_coco]])], dim=0)
    coco_normal1 = F.normalize(coco_list[1:], dim=1)

    defect_list = torch.zeros([1, 9])
    ...
    for file in defect_files:
        ...
        # concate成list
        defect_list = torch.cat([defect_list, torch.tensor([[ave_defect, std_defect, ju3_defect, u_defect, e_defect,
                                                                asm_defect, con_defect, ent_defect, idm_defect]])], dim=0)
    defect_normal1 = F.normalize(defect_list[1:], dim=1)

    similarity1 = torch.matmul(defect_normal1, coco_normal1.T)
    plt_(torch.mean(similarity1, dim=1).numpy())

    moli_list = torch.zeros([1, 9])
    moli_files = os.listdir(moli_path)
    for file in moli_files:
        file_path = os.path.join(moli_path, file)
        ave_moli, std_moli, ju3_moli, u_moli, e_moli = cedu_instance(file_path)
        asm_moli, con_moli, ent_moli, idm_moli = compute_instance(file_path, gray_level)

        # concate成list
        moli_list = torch.cat([moli_list, torch.tensor([[ave_moli, std_moli, ju3_moli, u_moli, e_moli,
                                                             asm_moli, con_moli, ent_moli, idm_moli]])], dim=0)
    moli_normal1 = F.normalize(moli_list[1:], dim=1)

    similarity2 = torch.matmul(defect_normal1, moli_normal1.T)
    plt_(torch.mean(similarity2, dim=1).numpy())

    # coco_normal = F.normalize(coco_list[1:] - torch.mean(coco_list[1:], dim=0).unsqueeze(0), dim=1)
    # defect_normal = F.normalize(defect_list[1:] - torch.mean(defect_list[1:], dim=0).unsqueeze(0), dim=1)
    # similarity = torch.matmul(defect_normal, coco_normal.T)